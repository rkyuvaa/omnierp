...
const ADV_STATUS_CONFIG = {
  ...
  settlement_manager_approved:   { label: 'Mgr Approved – Accts Review', color: '#f59e0b', bg: '#f59e0b20', icon: Clock },
  settlement_approved:           { label: 'Accounts Approved', color: '#3b82f6', bg: '#3b82f620', icon: CheckCircle },
  closed:                        { label: 'Settled', color: '#10b981', bg: '#10b98120', icon: CheckCircle },
};
...
export default function MyExpenses() {
  ...
  async function handleDeleteAdvance(id) {
    if (!window.confirm('Are you sure you want to delete this cash advance? This will also remove all associated ledger entries.')) return;
    try {
      await api.delete(`/expenses/advances/${id}`);
                          ...
                          >
                            ...
                            <td style={{ padding: '14px 18px', whiteSpace: 'nowrap' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                ...
                                {/* Settle Action — only visible AFTER accountant disburses (status = paid or later) */}
                                {['paid', 'settlement_pending', 'settlement_submitted', 'settlement_manager_approved', 'settlement_approved', 'closed'].includes(a.status) && (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      borderRadius: 6,
                                      border: 'none',
                                      background: ['paid', 'settlement_pending'].includes(a.status) ? 'var(--accent)' : 'var(--bg3)',
                                      color: ['paid', 'settlement_pending'].includes(a.status) ? '#fff' : 'var(--text2)',
                                      cursor: 'pointer',
                                      fontSize: 12,
                                      fontWeight: 700,
                                      display: 'flex',
                                  ...
                                  >
                                    <Wallet size={12} />
                                    {['paid', 'settlement_pending'].includes(a.status) ? 'Settle Expenses' : 'View Settlement'}
                                  </button>
                                )}
        ...
        {showAdvanceModal && (
          ...
          >
            ...
            >
              ...
              {/* Minimal Top Header Bar (Date, Purpose & Live Auto-Sum Total) */}
              <div
                style={{
                  display: 'grid',
              ...
              >
                ...
                <div>
                  ...
                  <textarea
                    placeholder="e.g. Advance required for supplier visit and travel expenses to Bangalore."
