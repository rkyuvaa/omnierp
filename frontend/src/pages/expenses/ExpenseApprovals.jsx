...
const ADV_STATUS_COLORS = {
  ...
  settlement_manager_approved:  '#f59e0b',
  settlement_approved:          '#3b82f6',
  closed:                       '#10b981',
};
  ...
  async function handleAction() {
    ...
    try {
      ...
      if (type === 'approve_claim') {
        ...
        toast.success('Settlement approved by Manager. Pending Accounts verification.');
      } else if (type === 'accounts_approve_settle') {
        await api.post(`/expenses/advances/${record.id}/settle/accounts-approve`, { remarks });
        toast.success('Settlement approved by Accounts. Ready for closure.');
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: `${color}15`, color, textTransform: 'uppercase' }}>
                              {a.status === 'settlement_submitted' ? 'SETTLEMENT SUBMITTED – PENDING MANAGER' :
                               a.status === 'settlement_manager_approved' ? 'MANAGER APPROVED – PENDING ACCOUNTS' :
                               a.status === 'settlement_approved' ? 'ACCOUNTS APPROVED – PENDING CLOSURE' :
                               a.status === 'closed' ? 'SETTLED' :
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                          ...
                          {/* Actions for settlements sheets (status: settlement_submitted) — Manager approval */}
                          ...
                          {/* Actions for Accounts verification (status: settlement_manager_approved) — Accountant only */}
                          {a.status === 'settlement_manager_approved' && isAuthorizedAccountant && (
                            <>
                              <button
                                onClick={() => setExpandedSettlement(expandedSettlement === a.id ? null : a.id)}
                                style={{
                              ...
                              >
                                {expandedSettlement === a.id ? (
                                  ...
                                  <>View Bills <ChevronDown size={12} /></>
                                )}
                              </button>
                              <button
                                onClick={() => setActionModal({ record: a, type: 'accounts_approve_settle' })}
                                style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: '#10b981', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}
                              >
                                <CheckCircle size={12} /> Accounts Approve
                              </button>
                              <button
                                onClick={() => setActionModal({ record: a, type: 'reject_settle' })}
                                style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #ef4444', background: '#ef444415', color: '#ef4444', cursor: 'pointer', fontWeight: 700, fontSize: 12 }}
                          ...
                          {/* Actions for Closure (Accountant only, when status is settlement_approved) */}
                          {a.status === 'settlement_approved' && isAuthorizedAccountant && (
                            <button
          ...
          >
            ...
            >
              <h3 style={{ margin: '0 0 18px', fontWeight: 800, fontSize: 16, color: 'var(--text)' }}>
                ...
                {actionModal.type === 'approve_settle' && '✓ Manager Approve Settlement'}
                {actionModal.type === 'accounts_approve_settle' && '✓ Accounts Verify & Approve Settlement'}
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    background:
                      ['approve_claim', 'approve_adv', 'approve_settle', 'accounts_approve_settle'].includes(actionModal.type)
                        ? '#22c55e'
                        : ['reject_claim', 'reject_adv', 'reject_settle'].includes(actionModal.type)
                        ? '#ef4444'
                        : actionModal.type === 'clarify_adv'
                ...
                >
                  {saving
                    ...
                    ? 'Approve'
                    : actionModal.type === 'accounts_approve_settle'
                    ? 'Accounts Approve'
                    : ['reject_claim', 'reject_adv', 'reject_settle'].includes(actionModal.type)
                    ? 'Reject'
                    : actionModal.type === 'clarify_adv'
