...
function FileField({ field, value, onChange }) {
  ...
  const handleDelete = async () => {
    if (value?.filename) {
      try {
        await fetch(`${baseUrl}/api/upload/${value.filename}`, {
  ...
  const isPdf = value?.content_type === 'application/pdf';
  ...
  return (
    <div style={{ width:'100%', boxSizing:'border-box' }}>
      ...
      {value?.filename ? (
        <div style={{ display:'grid', gridTemplateColumns:'1fr auto', alignItems:'center', gap:6, padding:'7px 10px', background:'var(--bg3)', borderRadius:8, border:'1px solid var(--border)', width:'100%', boxSizing:'border-box', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', gap:3, flexShrink:0 }}>
            {(isImage || isPdf) && (
              <button className="btn btn-ghost btn-sm" title="Preview" style={{ padding:'3px 5px' }}
                onClick={() => {
                  if (isPdf) {
                    const win = window.open('', '_blank');
                    win.document.write(`<html><head><title>${value.original_name}</title></head><body style="margin:0;background:#333"><embed src="${fileUrl}" type="application/pdf" width="100%" height="100%" style="position:fixed;top:0;left:0;width:100%;height:100%"/></body></html>`);
                  } else {
            ...
            <a href={fileUrl} download={value.original_name} className="btn btn-ghost btn-sm" title="Download" style={{ padding:'3px 5px' }}>
              <Download size={11}/>
            </a>
            <button className="btn btn-ghost btn-sm" title="Change file" style={{ padding:'3px 5px' }} onClick={() => inputRef.current?.click()}>
              <Upload size={11}/>
            </button>
            <button className="btn btn-danger btn-sm" title="Remove" style={{ padding:'3px 5px' }} onClick={handleDelete}>
              <X size={11}/>
            </button>
          </div>
        </div>
      ) : (
        <button className="btn btn-ghost" style={{ width:'100%', justifyContent:'center', border:'2px dashed var(--border)', borderRadius:8, padding:'10px', boxSizing:'border-box' }}
