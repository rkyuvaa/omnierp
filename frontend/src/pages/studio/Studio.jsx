...
const MODULES = ['crm', 'installation', 'service', 'warranty', 'konwertcare', 'payroll', 'expenses'];
    ...
    <Layout title="Studio — Layout & Workflow">
      <div className="flex gap-2 mb-6 items-center bg-white p-2 rounded-xl shadow-sm border border-gray-100">
        <div style={{ display: 'flex', gap: 4, background: 'var(--bg3)', padding: 4, borderRadius: 10 }}>
          {MODULES.map(m => (
            <button key={m} className={`btn btn-sm ${module === m ? 'btn-primary' : 'btn-ghost'}`} onClick={() => {
              setModule(m);
              if (['expenses', 'payroll'].includes(m)) setTab('sequences');
            }}>
              {m.toUpperCase()}
            </button>
          ))}
        ...
        <div className="ml-auto flex gap-2">
          {['layout', 'stages', 'sequences', 'documents']
            .filter(t => !(['expenses', 'payroll'].includes(module) && ['layout', 'stages', 'documents'].includes(t)))
            .map(t => (
              ...
              <div className="form-grid p-4">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" value={sequence?.suffix || ''} onChange={e => setSequence({ ...sequence, suffix: e.target.value })} placeholder="e.g. -A" />
                  </div>
                  <div className="form-group">
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="number" value={sequence?.current_number !== undefined ? sequence.current_number + 1 : 1}
                      onChange={e => setSequence({ ...sequence, current_number: Math.max(0, (parseInt(e.target.value) || 1) - 1) })} />
                  </div>
                </div>
                <div className="mt-4">
                  <button className="btn btn-primary" onClick={saveSequence}>Save Sequence</button>
                  <p className="text-muted text-sm mt-4">
                    Preview: <span className="ref-text">
                      {sequence?.prefix ? `${sequence.prefix}/` : ''}
                      {new Date().getFullYear()}/
                      {String((sequence?.current_number !== undefined ? sequence.current_number + 1 : 1)).padStart(sequence?.padding || 4, '0')}
                      {sequence?.suffix || ''}
                    </span>
                  </p>
                </div>
