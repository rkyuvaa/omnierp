...
def list_pending_advance_approvals(
    ...
    if is_acc_or_super:
        # Accountant / Super Admin sees everything that is pending approval (by manager) or pending action (by accountant)
        q = db.query(ExpenseAdvanceRequest).filter(
            ExpenseAdvanceRequest.status.in_(["submitted", "under_review", "settlement_submitted", "settlement_manager_approved", "approved", "settlement_approved"])
        )
    else:
        q = db.query(ExpenseAdvanceRequest).filter(
    ...
    try:
        ...
        for emp in employees:
            try:
                ...
                # Check if employee has a pending settlement review
                pending_review_adv = db.query(ExpenseAdvanceRequest).filter(
                    ExpenseAdvanceRequest.employee_id == emp.id,
                    ExpenseAdvanceRequest.status.in_(["settlement_submitted", "settlement_manager_approved"])
                ).first()
                has_pending_review = pending_review_adv is not None
...
def update_employee_opening_balance(
    ...
    db.commit()
    recalculate_ledger_balances(db, emp.id)
...
def _next_advance_ref(db: Session) -> str:
    from app.auth import next_sequence
    return next_sequence(db, "expenses")
...
def recalculate_ledger_balances(db: Session, employee_id: int):
    """Recalculate the running balance of all ledger transactions for an employee chronologically."""
    txs = db.query(ExpenseAdvanceLedger).filter(
        ExpenseAdvanceLedger.employee_id == employee_id
    ).order_by(ExpenseAdvanceLedger.created_at.asc(), ExpenseAdvanceLedger.id.asc()).all()
    
    current_balance = 0.0
    for tx in txs:
        if "opening balance" in (tx.description or "").lower():
            current_balance = float(tx.amount or 0.0)
        else:
...
def payout_advance_request(
    ...
    if accountant_id:
        from app.auth import get_current_employee_optional
        emp = get_current_employee_optional(current_user, db)
        if emp and emp.id == accountant_id:
            is_auth_accountant = True
            
    if not is_auth_accountant and not current_user.is_superadmin:
        if not accountant_id and is_hr_admin(current_user, db):
            pass
        else:
            raise HTTPException(403, "Only the configured Accountant or Super Admin can disburse payouts")
...
def submit_settlement(
    ...
    if advance.status not in ("approved", "paid", "settlement_pending"):
...
def approve_settlement(
    ...
    """Step 2: Manager approves settlement → settlement_manager_approved. Notifies accountant."""
    advance = db.query(ExpenseAdvanceRequest).filter(ExpenseAdvanceRequest.id == adv_id).first()
    if not advance:
        raise HTTPException(404, "Advance request not found")
    ...
    # Mark L1 (manager) as approved
    if emp and advance.l1_approver_id == emp.id:
        advance.l1_status = "approved"
        advance.l1_remarks = data.remarks
        advance.l1_approved_at = datetime.utcnow()
    elif is_admin:
        # Admin override — treat as manager-level approval
        advance.l1_status = "approved"
        advance.l1_remarks = data.remarks
        advance.l1_approved_at = datetime.utcnow()
    else:
    ...
    # Move to intermediate status: pending accountant verification
    advance.status = "settlement_manager_approved"
    advance.approver_remarks = data.remarks
    advance.approved_at = datetime.utcnow()
    ...
    if advance.employee and advance.employee.user_id:
        _notify_advance(db, advance.employee.user_id,
                        "Settlement Manager Approved ✓",
                        f"Your expense settlement for advance {advance.reference} has been approved by Manager. Pending Accounts verification.",
                        advance.id)
    ...
    if setting and isinstance(setting.value, dict):
        accountant_id = setting.value.get("accountant_id")
    if accountant_id:
        accountant_emp = db.query(HREmployee).filter(HREmployee.id == accountant_id).first()
        if accountant_emp and accountant_emp.user_id:
            _notify_advance(db, accountant_emp.user_id,
                            "Settlement Pending Accounts Verification",
                            f"{advance.employee.name if advance.employee else 'Employee'}'s settlement for {advance.reference} was approved by Manager. Please verify and approve.",
                            advance.id)
            
    db.commit()
    return {"message": "Settlement approved by Manager. Pending Accounts Manager verification.", "id": advance.id}


@router.post("/advances/{adv_id}/settle/accounts-approve")
def accounts_approve_settlement(
    adv_id: int,
    data: AdvanceAction,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Step 3: Accounts Manager (Accountant) verifies and gives final approval → settlement_approved."""
    advance = db.query(ExpenseAdvanceRequest).filter(ExpenseAdvanceRequest.id == adv_id).first()
    if not advance:
        raise HTTPException(404, "Advance request not found")
        
    if advance.status != "settlement_manager_approved":
        raise HTTPException(400, f"Settlement is not pending Accounts verification. Current status: {advance.status}")
    
    # Check accountant authorization
    setting = db.query(SystemSetting).filter(SystemSetting.key == "expenses_settings").first()
    accountant_id = None
    if setting and isinstance(setting.value, dict):
        accountant_id = setting.value.get("accountant_id")
    ...
    if not is_auth_accountant and not current_user.is_superadmin:
        if not accountant_id and is_hr_admin(current_user, db):
            pass  # Allow HR admin if no accountant configured
        else:
            raise HTTPException(403, "Only the configured Accountant or Super Admin can give final settlement approval")

    # Mark L2 (accountant) as approved
    advance.l2_status = "approved"
    advance.l2_remarks = data.remarks
    advance.l2_approved_at = datetime.utcnow()
    
    # Move to final approval status — ledger update deferred to close stage
    advance.status = "settlement_approved"
    advance.approver_remarks = data.remarks
    
    if advance.employee and advance.employee.user_id:
        _notify_advance(db, advance.employee.user_id,
                        "Settlement Fully Approved ✓",
                        f"Your expense settlement for advance {advance.reference} has been verified and approved by Accounts. Settlement will be processed shortly.",
                        advance.id)
            
    db.commit()
    return {"message": "Settlement approved by Accounts Manager. Ready for closure.", "id": advance.id}
...
def close_advance(
    ...
    if not is_auth_accountant and not current_user.is_superadmin:
        if not accountant_id and is_hr_admin(current_user, db):
            ...
            raise HTTPException(403, "Only the configured Accountant or Super Admin can settle/close advances")
    ...
    # 1. Log the debit transaction for the total claimed expenses
    _log_ledger_transaction(
        db=db,
        employee_id=advance.employee_id,
    ...
    # 2. Log any difference (return or reimbursement credit)
    if diff > 0:
        _log_ledger_transaction(
            db=db,
            employee_id=advance.employee_id,
...
def delete_advance_request(
    ...
    if not is_hr_admin(current_user, db):
        raise HTTPException(403, "Admin / Accountant access required to delete advances")
    ...
    db.query(ExpenseAdvanceLedger).filter(ExpenseAdvanceLedger.advance_id == adv_id).delete()
    
    # Delete the request (cascades to settlement lines)
    db.delete(advance)
    db.commit()
    
    recalculate_ledger_balances(db, employee_id)
    return {"message": "Advance request and associated ledger entries deleted successfully"}
...
def delete_ledger_transaction(
    ...
    if not is_hr_admin(current_user, db):
        raise HTTPException(403, "Admin / Accountant access required to delete ledger transactions")
        
    tx = db.query(ExpenseAdvanceLedger).filter(ExpenseAdvanceLedger.id == tx_id).first()
    if not tx:
        raise HTTPException(404, "Ledger transaction not found")
...
class ExpensesSettingsSchema(BaseModel):
    accountant_id: Optional[int] = None

@router.get("/settings")
def get_expenses_settings(db: Session = Depends(get_db)):
    setting = db.query(SystemSetting).filter(SystemSetting.key == "expenses_settings").first()
    if not setting or not isinstance(setting.value, dict):
        return {"accountant_id": None}
    return setting.value

@router.post("/settings")
def save_expenses_settings(
    data: ExpensesSettingsSchema,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    if not is_hr_admin(current_user, db):
        raise HTTPException(403, "Admin access required to save expenses settings")
    ...
    if setting:
        ...
        setting = SystemSetting(key="expenses_settings", value=data.dict())
        db.add(setting)
    db.commit()
    return {"message": "Expenses settings saved successfully", "settings": setting.value}
